...
import Program from "./editComponents/Program";
import Address from "./editComponents/Address";
import Price from "./editComponents/Price";
import { URL_API } from "../../../constants";

axios.defaults.withCredentials = true;

function EditForm(props) {
  const { onSelectFile, selectedFile, tour } = props;
  ...
  const [avaiableTour, setAvainableTour] = useState("");
  ...
  useEffect(() => {
    const test = () => {
      setTitle(tour.title);
      setDaytime(tour.time.charAt(0));
      setNights(tour.time.charAt(0) == 1 ? [1] : [tour.time.charAt(0) - 1, tour.time.charAt(0)]);
      setNight(tour.time.charAt(7));
      setRegion(tour.region);
      setCity(tour.city);
      setTotalTour(tour.totalTour);
      setAvainableTour(tour.avaiableTour);
      setTransport(tour.transport);
      setDateStart(tour.dateStart);
      setPrice(tour.price);
      setSaleOff(tour.saleOff);
      setTime(tour.time);
      programs.current = tour.program;
      id.current = tour._id;
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`${URL_API}/company/tour/update`, formData, {
        headers: {
  ...
  return (
    <form action="" onSubmit={handleSubmit}>
      <Grid container spacing={2}>
        ...
        <Program boolean={boolean} setBoolean={setBoolean} daytime={daytime} programs={programs} />
        ...
        <Grid item xs={6}>
          <TextField
            ...
            variant="outlined"
            fullWidth
            focused={avaiableTour ? true : false}
            sx={{
              mb: 2,
              "& label": { fontSize: "1.4rem", paddingRight: 1, backgroundColor: "#f0f2f5" },
            }}
            onChange={(e) => setAvainableTour(e.target.value)}
          />
        </Grid>
